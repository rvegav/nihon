...
<div class="card">
								...
								<div class="row">
									<div class="col-md-6">
										...
										<div class="input-group">
											<input type="hidden" name="empl_id" id="empl_id" value="<?php echo $usuario->usua_empl_id ?>">
											<input type="text" name="" id="empleado" placeholder="Buscar Empleado" class="form-control" disabled="disabled" value="<?php echo $usuario->usua_empleado ?>">
									...
									<div class="col-md-6">
										...
										<div class="input-group">
											<input class="form-control" type="text" name="username" placeholder="Usuario" value="<?php echo $usuario->usua_name ?>">
										</div>
									</div>
								</div>
								<br>
												...
												<!-- <div class="table-wrapper-scroll-y my-custom-scrollbar"> -->
													<table class="table table-bordered" id="roles_seleccionados">
														...
														<tbody>
															<?php if ($detalles_rol): ?>
																<?php $item = 1 ?>
																<?php foreach ($detalles_rol as $detalle): ?>
																	<tr>
																		<td><?php echo $item ?></td>
																		<td><?php echo $detalle->rol_descripcion?></td>
																		<input type="hidden" id="modulo" name="roles[]" value="<?php echo $detalle->rol_id ?>" >
																		<td><button type="button" class="btn btn-danger quitar_rol" value="<?php echo $detalle->rol_id ?>"><i class="fa fa-trash"></i></button></td>
																	</tr>
																<?php endforeach ?>
															<?php endif ?>
														</tbody>
	...
	<script type="text/javascript">
		...
		$("#btn-agregar").on("click", function(){
			if ($("SELECT#roles option:selected").val()!='') {
				console.log($('#roles_seleccionados tr.data').map());
				item++;
				html = '<tr>';
				html += '<td>';
				html += item;
				...
				html += '</td>';
				html += '<td>';
				html += '<button type="button" class="btn btn-danger quitar_rol"><i class="fa fa-trash"></i></button>';
				html += '</td>';
				html += '<tr>'
		...
		$('#form-user').submit(function(event) {
			...
			$.ajax({
				url: "<?php echo base_url()?>update_usuario",
				type: 'POST',
				data: formDato,
			})
		...
		$('#roles_seleccionados tbody').on('click', '.quitar_rol', function (event) {
