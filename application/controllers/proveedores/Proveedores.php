...
class Proveedores extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('Proveedores_model', 'Ciudad_model'));
	...
	public function store()
	{
		...
		if ($this->form_validation->run() == FALSE){
			...
			$idciudad = $this->Proveedores_model->ObtenerCodigo();
			$time = time();
	...
	{
		$data = array(
			'ciudad'=> $this->Proveedores_model->getCiudades($id),
		);
	...
	public function update()
	{
		...
		$estado = $this->input->post('estado');
			...
			if ($this->form_validation->run() == FALSE){
				...
				if($this->Proveedores_model->update($idCiudad,$data)){
					$mensajes['correcto'] = 'correcto';
